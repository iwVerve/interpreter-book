...
pub fn evalReturnStatement(self: anytype, statement: ast.ReturnStatement, environment: *Environment) !Value {
    const value = try self.evalExpression(statement.expression, environment);
    self.return_state = .function;
    return value;
}

pub fn evalLetStatement(self: anytype, statement: ast.LetStatement, environment: *Environment) !Value {
    const value = try self.evalExpression(statement.expression, environment);
    try environment.set(statement.identifier.name, value);
    return value;
}

pub fn evalStatement(self: anytype, statement: ast.Statement, environment: *Environment) InterpreterError!Value {
...
pub fn evalStatements(self: anytype, statements: ast.BlockStatement, environment: *Environment) !Value {
